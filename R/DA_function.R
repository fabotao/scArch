...
run_hierarchical_DA <- function(
  ...
  clust.df <- data.frame("cell_id"=colnames(obj), "Louvain.Clust"=cluster.layers[,dim(cluster.layers)[2]])
  ...
  for(kk in 1:dim(logFC.matrix)[2]){
    ...
    DA.layer = do.call(rbind, tapply(1:dim(cluster.layers)[1], cluster.layers[,kk], function(x){
      ...
      louvain.count <- table(clust.df.x$Louvain.Clust, clust.df.x$Sample)
      ...
      if(is.null(fit) | is.null(dge)){
        len = dim(louvain.count)[1]
        louvain.res = data.frame(logFC=rep(0, len),
                                 logCPM=rep(0,len),
                                 F=rep(0, len),
                                 PValue=rep(1,len),
                                 FDR=rep(1,len))
        rownames(louvain.res) = rownames(louvain.count)
        return(louvain.res)
